...
/** Display ADA account balance */
async function printETHBalance({ address, name }) {
  console.log(`${name} ADA balance is`, web3.utils.fromWei(await web3.eth.getBalance(address)))
}
...
async function withdraw({ deposit, currency, amount, recipient, relayerURL, refund = '0' }) {
  if (currency === 'ADA' && refund !== '0') {
    throw new Error('The ADA purchase is supposted to be 0 for ADA withdrawals')
  }
  refund = toWei(refund)
  if (relayerURL) {
...
function convert_timestamp(_timestamp){
...
async function init({ rpc, noteNetId, currency = 'dai', amount = '100' }) {
  ...
  if (inBrowser) {
    ...
    try {
      if (web3 == undefined) {
        ...
        contractJson = await (await fetch('build/contracts/ADASmashnado.json')).json()
    ...
    if (web3 === undefined) {
      ...
      contractJson = require('./build/contracts/ADASmashnado.json')
  ...
  erc20 = currency !== 'ada' ? new web3.eth.Contract(erc20ContractJson.abi, tokenAddress) : {}
}

async function main() {
  if (inBrowser) {
    const instance = { currency: 'ada', amount: '0.1' }
    //await init(instance)
